...
1. ~~**Který git provider?**~~ **Rozhodnuto: GitHub.** Tím se upřesnilo i omezení „žádný SaaS“ — týká se **CMS a obsahu**, ne vývojové infrastruktury. Důsledky: GitHub Actions jako CI runner, zápis obsahu přes GitHub API, a **Cloudflare R2 pro média je tím taky přípustné**. (Přihlášení do Tiny nakonec neběží přes GitHub — viz bod 4.)
2. ~~**Kam s médii?**~~ **Rozhodnuto: zůstávají v gitu.** Zvažovaly se **Cloudflare R2** (levné, nulové poplatky za odchozí data, nic se nespravuje) a **MinIO na vlastním serveru** (o jednu službu víc k údržbě). Rozhodlo, že **git má média replikovaná v každém klonu a nikdo je nemusí zálohovat zvlášť** — u 182 PDF s vydanými dokumenty organizace to váží víc než velikost repozitáře. Migrace nakonec přenesla 531 souborů (291 MB), ne 1 377, protože se bralo jen to, na co obsah odkazuje.

   Cena je dvojí: `.git` má 277 MB, což zdržuje každý CI checkout, a **knihovnu médií si musel projekt napsat sám** — self-hosted TinaCMS media handler nedodává (`src/lib/media.ts`, `src/pages/api/media/`, `tina/media-store.ts`). Kdyby média někdy narostla do jednotek GB nebo měl web stát za CDN v jiném regionu, přechod na R2 je výměna jedné implementace `MediaStore`, ne přepis.
...
4. ~~**Jak se redaktoři přihlašují?**~~ ~~**GitHub OAuth.**~~ **Přehodnoceno: účtem Google Workspace přes better-auth** — organizace má pracovní účty u Googlu tak jako tak a odchod z organizace přístup odebere sám, takže redakce nemusí zakládat účty na GitHubu. Cenou je, že obsah commituje jeden servisní účet a v historii repozitáře nejsou vidět jednotliví redaktoři. Podrobně v [14-autentizace.md](14-autentizace.md).
5. ~~**Zálohy Postgresu a obnova.**~~ ~~**Index je v MongoDB, ne v Postgresu.**~~ **Rozhodnuto: index je SQLite soubor a databázový server žádný** — Tina bere jako index libovolnou implementaci `abstract-level` a `sqlite-level` je jedna z nich (vydává ho tým TinaCMS). Index drží obsah odvozený z gitu, takže ztráta není fatální: `tinacms build` ho postaví znovu — ověřeno, 4,2 MB a součást každého buildu. Rozbor v [15-tinacms-vs-decap.md](15-tinacms-vs-decap.md) sekci 9, postup v [16-migrace-sqlite.md](16-migrace-sqlite.md).
